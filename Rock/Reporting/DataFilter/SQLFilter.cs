...
namespace Rock.Reporting.DataFilter
{
    ...
    public class SQLFilter : DataFilterComponent, IDataFilterRequireFilterId
    {
        #region Properties
...
";
        ...
        {
            return GetExpression( entityType, serviceInstance, parameterExpression, selection, 0 );
        }
        ...
        /// <param name="selection">The selection.</param>
        /// <returns></returns>
        public Expression GetExpression( Type entityType, IService serviceInstance, ParameterExpression parameterExpression, string selection, int filterId )
        {
            if ( selection.IsNotNullOrWhiteSpace() )
            {
                ...
                //On preview we just take the first 1000
                if ( filterId == 0 && entityIds.Count > 1000 )
                {
                    entityIds = entityIds.Take( 1000 ).ToList();
                }
                ...
                using ( RockContext updateContext = new RockContext() )
                {
                    ...
                    var storedEntityIds = sqlStores.Where( s => s.DataviewFilterId == filterId ).Select( s => s.EntityId ).ToList();
                    ...
                    {
                        updateContext.Database.ExecuteSqlCommand( string.Format( "DELETE FROM DataViewSQLFilterStore WHERE DataViewFilterId = {0}", filterId ) );
                        toAdd = entityIds;
                    }
                    else if ( toRemove.Any() )
                    {
                        updateContext.Database.ExecuteSqlCommand( string.Format( "DELETE FROM DataViewSQLFilterStore WHERE DataViewFilterId = {0} AND EntityId IN  ({1})", filterId, string.Join( ",", toRemove ) ) );
                    }
                ...
                var selectedEntityIds = rockContext.DataViewSQLFilterStores.Where( s => s.DataviewFilterId == filterId ).Select( s => s.EntityId );
